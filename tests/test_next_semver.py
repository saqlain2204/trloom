...
from next_semver import bump_kind, format_tag, latest_version, next_version  # noqa: E402
...
def test_bump_kind_from_message() -> None:
    assert bump_kind("Merge PR\n\n#major") == "major"
    assert bump_kind("feat: something [minor]") == "minor"
    assert bump_kind("fix typo") == "patch"
    assert bump_kind("chore: docs [skip release]") is None
    assert bump_kind("chore: docs [skip tag]") is None
...
def test_cli_patch_bump() -> None:
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "--message", "merge", "--tags", "v0.1.1"],
        check=True,
        capture_output=True,
        text=True,
    )
...
def test_cli_skip() -> None:
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "--message", "docs [skip release]", "--tags", "v0.1.1"],
        capture_output=True,
        text=True,
    )
...
def test_cli_minor() -> None:
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "--message", "feat [minor]", "--tags", "v0.1.1"],
        check=True,
        capture_output=True,
        text=True,
    )
